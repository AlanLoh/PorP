...
RANDOM_GENERATOR = np.random.default_rng(seed=42) # no seed = non-reproducible
...
def generate_domain_matrix(matrix_side: int, n_domains: int) -> np.ndarray:
    ...
    for i in range(matrix_side):
        for j in range(matrix_side):
            ...
            domain_map[i, j] = RANDOM_GENERATOR.choice(domains, size=1, p=domain_probability/domain_probability.sum())[0]

    return domain_map

def assign_int_to_matrix(matrix: np.ndarray, n_symbols: int) -> None:
    ...
    for domain_i in range(n_domains):
        ...
        matrix[domain_mask] = domain_i + RANDOM_GENERATOR.choice(np.linspace(0.1, 0.9, n_symbols), size=domain_size)
    return matrix

def assign_symbol_to_matrix(matrix: np.ndarray, symbols: dict) -> None:
    n_domains = np.unique(matrix).size
    for domain_i in range(n_domains):
        domain_mask = matrix == str(domain_i)
        domain_size = np.sum(domain_mask)
        matrix[domain_mask] = RANDOM_GENERATOR.choice(symbols[domain_i], size=domain_size)
    return matrix
...
def generate_card_list_from_domain_dict(domain_dict: dict, n_cards: int = 100, n_symbols_per_card: int = 5) -> List[Card]:
    ...
    # Loop over n_symbols_per_card and generate as many maps
    symbol_map = np.empty((map_side, map_side, n_symbols_per_card), dtype=str)
    for i in range(n_symbols_per_card):
